 ...
 *                     (Bazarr badges, the Bazarr tab). Structurally safe: any
 *                     pool failure degrades to "no cached data", never a
 *                     500, because this sits directly on the badge/page
 *                     render path.
...
final class StaleWhileRevalidateCache
{
    ...
    public function getOrCompute(string $key, int $softTtl, int $hardTtl, callable $fetch): array
    {
        ...
        // PHPStan infers $env's type from the callback above and would
        // therefore flag the shape check below as "always true" — but a
        // cache HIT skips that callback entirely and hands back whatever was
        // already stored under this key (an older build's shape, or a
        // stand-in CacheInterface in a test), so the static type is not a
        // runtime guarantee. Widen back to mixed so the real, necessary
        // check below is not treated as dead code.
        /** @var mixed $env */
        if (!is_array($env) || !isset($env['fetchedAt']) || !array_key_exists('value', $env)) {
        ...
        if ($env['value'] !== []) {
            ...
            // MediaLibraryRefresher / guardrail 6) is likewise only ever
            // called on a non-empty fetch. See refreshIsOverdue().
            $this->cacheApp->deleteItem($key . '.requested_at');
        }

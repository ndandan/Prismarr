...
class GlobalSearchNormalizationTest extends TestCase
{
    ...
    private function makeController(array $movies, array $series = []): MediaController
    {
        ...
        // Bazarr not configured: every lookup answers the `hidden` shape, so
        // no `subtitle` key reaches the payload and these cases stay about
        // normalization/ranking only.
        $subtitles = $this->createMock(BazarrSubtitleIndex::class);
        $subtitles->method('movieStatus')->willReturn(['state' => 'hidden', 'count' => 0]);
        $subtitles->method('seriesStatus')->willReturn(['state' => 'hidden', 'count' => 0]);

...
### Fixed
- **Radarr / Sonarr library pages report the instance unreachable even though the connection test passes** ([#41](https://github.com/Shoshuo/Prismarr/issues/41)). The heavy `getMovies()` / `getSeries()` payload is fetched through the shared `get()`, whose total timeout is capped at 8 s (Radarr) / 4 s (Sonarr). On a large library or slow storage the full `/movie` / `/series` response can exceed that, so the call times out → the instance is marked down and the breaker short-circuits the rest of the page ("unreachable"), while the lightweight connection *test* hits a fast endpoint and still passes. The heavy library calls now use a generous 30 s total timeout (`getMovies()` / `getSeries()` / `getRawAllSeries()`); the connect timeout stays at 4 s everywhere so a genuinely down instance still trips the breaker fast. If a fetch still times out at 30 s the instance itself is stuck (scan / RSS / slow storage), not Prismarr.

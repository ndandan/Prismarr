...
- **In-place quick-look on the dashboard.** Clicking any media tile — the hero spotlight, a recently-added title, an upcoming/calendar entry, a weekly-trending pick or a watchlist item — now opens a read-only detail modal *in place* on the dashboard instead of navigating to another page. Library items (Radarr/Sonarr) show poster/backdrop, title and year, a downloaded / monitored / missing status badge, rating, runtime (movies) or network (series), genres and synopsis, plus a "Manage on Radarr/Sonarr →" deep-link into the full editor; TMDb items (trending / watchlist) show the same card with a "View in Discover →" link and no library status. The modal is fed by two fail-open fragment endpoints: library detail reuses the dashboard's existing 45 s library cache (usually zero extra upstream calls) and falls back to a single `getMovie` / `getSerie`, while TMDb detail uses the 1 h-cached `TmdbClient`. Every tile keeps its original link as a progressive-enhancement fallback, so a disabled-JS or failed-fetch click simply navigates to the relevant tab as before. Strings are i18n'd (en/fr).

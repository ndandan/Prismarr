...
### Added
- **Episode quality and custom-format score in the series episode list** ([#39](https://github.com/Shoshuo/Prismarr/issues/39)). The downloaded quality was only visible by hovering the per-episode info icon, and the custom-format score wasn't shown at all. Each downloaded episode now shows its quality as a badge inline in the list, plus the custom-format score when it is non-zero (coloured green/red, mirroring the interactive-search table; a score of 0 is hidden to avoid a column of zeros). The info-hover tooltip still carries the full media detail (codec, resolution, audio, size, release group). No API changes — Sonarr's episode-file payload already includes the quality, custom formats and score.

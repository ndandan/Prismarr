...
### Added
- **Default quality profile per Radarr / Sonarr instance** ([#5](https://github.com/Shoshuo/Prismarr/issues/5)). Each instance's edit form in `/admin/settings` gains a "Default quality profile" picker (its options load live from that instance's Radarr/Sonarr when the modal opens). When set, that profile is preselected in the quick-add, suggested-add and library add forms instead of whichever profile the service happens to return first; leaving it on "None" keeps the previous behaviour, so existing installs are unaffected. The default is stored per instance (a new nullable `default_quality_profile_id` column + migration) because profile ids aren't shared across instances, and a server-side fallback applies it to programmatic adds that don't specify a profile.

...
- **Tautulli Plex Activity page enhancements.** The activity page gained a stream-summary strip above Now Playing (session count, Direct Play / Direct Stream / Transcode breakdown, total/LAN/WAN bandwidth — reusing the dashboard widget's summary via a shared partial) and a "Recently added" section (the latest 10 items added to Plex, each clickable into the same info modal). The live stream cards — shared with the dashboard widget — now also show an HDR/SDR dynamic-range badge and, on transcoding streams, the source→target codec transition (e.g. `Video: HEVC → H264`) instead of just the decision word. New data comes from the read-only `get_recently_added` command plus already-sanitized session fields; the new section fails open like the rest of the page.

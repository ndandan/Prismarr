...
### Fixed
- **Import scans, statistics and rename pages no longer treat a slow library as an empty one.** When the 30 s full-library budget became opt-in (`LIBRARY_TIMEOUT`, entry below), six call sites outside the library routes were left on the short 4 s / 8 s defaults — and a timed-out fetch returns an empty list indistinguishable from an empty library. The sharpest consequence: the Radarr/Sonarr library-import scan would report every already-imported folder as new (inviting a full duplicate re-import), while the statistics pages rendered "0 series / 0 movies" as authoritative and the torrent-resolver quietly answered "no match in library". Those six sites (import scan, statistics and rename in both controllers, plus the torrent resolver's series sweep) now pass `LIBRARY_TIMEOUT` again, and a source-sentinel test pins them.
- **Sonarr health warnings are escaped in the series page banner.** The auto-refreshing warnings banner spliced Sonarr's warning text raw into `innerHTML` while the films sibling already escaped the same data — markup in a warning string would have been injected into the page. Guard row added to the escaping sentinel.
- **Prowlarr settings values containing a double quote no longer corrupt on save.** The quote-escaping fix for the Prowlarr indexer page never reached its three sibling pages (applications, download clients, notifications): their `esc()` copies escape `& < >` but not `"`, and their output lands in `value="…"` attributes — so a field like `--user="bob"` rendered truncated at the first quote, and pressing Save posted the truncated value back to Prowlarr. All copies now match the hardened version.
- **Episode air dates no longer shift a day for users west of UTC.** Sonarr's `airDate` is a floating broadcast date deliberately anchored at midnight UTC (the issue-#26 fix), but the date-preference sweep converted it to the user's display timezone — so a Monday premiere rendered as Sunday for any negative-offset zone. A new `floating` mode of the date filter formats such calendar dates without timezone conversion (still honoring the date-format preference); the upcoming-episodes row also drops the time half, which was the anchor's meaningless midnight. Applied to the series calendar card and the missing/cutoff tables.
- **Blocked-import warnings are visible without expanding the queue.** The series download queue became collapsible with its body hidden by default, but the "N blocked imports" banner — with its Clean-duplicates and Import-files buttons — was injected *inside* that hidden body, so stuck imports were never surfaced unless the user happened to expand the queue. The banner now sits between the card header and the collapsed body.
- **The global Deluge and Transmission download pollers ran twice on every page.** A rebase applied each poller block to the base template twice; both copies rendered and both self-started, double-firing a poll per page load, racing each other's saved snapshot in `localStorage`, and the two Deluge copies had already drifted apart. The duplicated pair is removed (−345 lines) and a structure sentinel now fails the build if a second copy ever returns — closing the "duplicated badge pollers" caveat noted in the Turbo-lifecycle entry below.
- **A slow Radarr/Sonarr payload no longer marks the whole instance down.** `get()`/`multiGet()` ran the same breaker branch for a read timeout after a successful connect as for a connection failure, so a fetch that outlived its budget marked the instance down and short-circuited every caller to "unreachable" for the breaker window (issue #41's root failure mode — the earlier 30 s widening only moved the threshold). The two are now distinguished via `curl_errno` + `CURLINFO_CONNECT_TIME`: slow-payload timeouts fail only that call (and still short-circuit the remainder of the same request), while connect-level failures trip the breaker exactly as before. The 30 s full-library budget is also now opt-in per call site (`LIBRARY_TIMEOUT`): the library/collections/filter/search-index routes pass it, while dashboard widgets, quick-look membership checks and the admin statistics loop return to the 8 s / 4 s bound so one slow instance can't pin their requests for 30 s each.
- **The mobile filter drawer kept half-committed state when dismissed without Apply.** The pill/select handlers write the form controls immediately while navigation waits for Apply, so closing the drawer via X/backdrop left the drawer disagreeing with the grid — and the stale values silently rode along on the next submit (including a native Enter-in-search submit). A `hidden.bs.offcanvas` handler now resets the form to its server-rendered (committed URL) values and resyncs the status pills.
- **Back-navigation could restore a films/series page with the filter drawer stuck open.** Apply navigates while the drawer is open, so Turbo snapshotted the page with `.offcanvas.show` and a live backdrop; a Back restore resurrected a dimmed, scroll-locked page whose backdrop had no bound Offcanvas instance. `turbo:before-cache` now scrubs offcanvas state before snapshotting (and the existing `turbo:before-render` sweep does the same).
- **Deluge and Transmission were labeled "Usenet — optional" in the setup wizard.** Both torrent-client sections carried the Usenet suffix that belongs to SABnzbd/NZBGet; they now match qBittorrent's plain header.
- **A typo in `PRISMARR_WORKER_NUM` could brick the container, and junk `PRISMARR_WORKER` values silently enabled worker mode.** The worker count is interpolated verbatim into the Caddyfile's global `frankenphp {}` block, so a non-integer value failed Caddy's parse and restart-looped the whole app; it is now accepted only as a pure positive integer (anything else is ignored with a warning). And the mode switch was fail-open — any unrecognized value (`disabled`, `none`, `"OFF "` with a stray space) enabled worker mode; it now requires an explicit truthy value and fails closed with a warning otherwise.
- **`TmdbClient::regionPriority()` could rank the wrong country first.** An explicit region subtag was discarded (`en_GB` still led with US — it now leads with the named country), and an unmapped UI language was upcast to a "country" of the same letters ('sv' Swedish → SV = El Salvador; ja/ko/uk → invalid codes). Latent while the UI ships only en/fr, both fixed with regression tests.
- **`/search/online` returned a raw 500 on a cold cache.** The online-search action passed a fake cache callback — `fn(ItemInterface $item) => ($item->expiresAfter(60)) ?: []` — but `expiresAfter()` is fluent and returns `$this`, so the `?:` fallback was unreachable: on a miss the callback handed back the cache item itself, `array_column()` received a non-array and threw a `TypeError`, and the `ItemInterface` was written into the pool for 60 s on top of that. The reliable way to hit it was to save any admin setting (which clears `cache.app`) and then use `Ctrl+K` — the very next online search 500'd. Both `/search` and `/search/online` now share the same real index builder, so a miss builds the index instead of poisoning the cache with an object.
- **Four integrations failed silently in the logs.** `TautulliClient::recordError()` swallowed the very failure it is named for without emitting anything, and the Gluetun, Unraid, UniFi and Houndarr clients logged their transport and decode failures at `debug` — a level the production Monolog config never writes — so a misconfigured integration rendered its muted offline state with no trace anywhere to triage from. `recordError()` now warns, and 11 `debug` calls are promoted to `warning` across those clients. All of them sit behind the existing enabled/configured guards, so a service you never set up still logs nothing.
- **Per-page timers and handlers leaked across Turbo navigations.** The base health poller, the Plex pill and the Films, Series and Discover pages each registered their `turbo:load` work unguarded, so handlers stacked once per visit and their intervals outlived the page they belonged to. The user-visible proof: visiting Discover *K* times and then clicking the watchlist star fired *K* POSTs — add, then remove, then add — and the star silently reverted. A shared `window.registerPageLifecycle(init)` helper now runs `init` at parse time and invokes the teardown it returns from a self-removing `turbo:before-render` listener, so each page mounts exactly once; the star now fires exactly one POST after any number of visits. This promotes a pattern thirteen templates already hand-rolled correctly. The duplicated badge pollers in `base.html.twig` and the cross-template duplicate element ids are still outstanding.
- **`app.js` imported its stylesheet from JavaScript, which the strict CSP then killed.** AssetMapper compiles an `import './styles/app.css'` into a `data:application/javascript` stub module — and `script-src` no longer allows `data:`, so the browser refused the stub and, with it, the whole module graph hanging off it: Turbo, Stimulus (including the stateless-CSRF cookie controller), Alpine and Chart.js. The page rendered and every piece of scripted behaviour was simply gone. The stylesheet is now loaded with a plain `<link>`, and a guard test fails the build if a CSS import ever returns to `app.js`.
- **Main images reported a stale, hardcoded version.** The GHCR workflow stamped every `main` build `1.1.0-tautulli` regardless of what it actually contained, so `:latest` misreported itself in the About and Updates cards indefinitely. Main builds now stamp `main-<short sha>` and additionally carry the full commit SHA (`PRISMARR_GIT_SHA`, also emitted as the OCI `org.opencontainers.image.revision` label) — which is what makes the new commits-behind check possible at all. Branch dispatches keep their existing `beta-<branch>` string; a local build with no SHA stamped simply hides the check.
...
- **Subtitle badges, the Bazarr tab and the Radarr/Sonarr library lists no longer block a page render on a slow external refill.** Films, quick-look, Ctrl+K search and the Bazarr tab all read a shared stale-while-revalidate cache instead of fetching Bazarr/Radarr/Sonarr inline, so a cold cache renders from whatever's on hand while the refresh runs out-of-band in the existing `messenger-worker` consumer (previously-dormant plumbing; its recycle threshold is raised from 256M to 512M to hold a ~5,382-row Bazarr decode without restarting every refresh). The previous 7–11 s stalls on the first Films, quick-look, search or Bazarr-tab visit each minute are gone — Films/Series badges, the quick-look and search badges, and the Wanted/Movies/Series tab views no longer wait on Bazarr. (The Bazarr History list and the series drill-down's episode subtitle chips are unchanged: both still fetch Bazarr inline on open, since neither is a badge rendered across a whole grid.) A subtitle badge with genuinely no cached data yet (typically right after a deploy) now shows a muted, non-interactive "…" pending chip for one refresh cycle instead of either blocking the page or silently claiming "nothing missing"; a subtitle download or auto-search still updates that item's own badge immediately regardless of what the background refresh is doing. If the background worker is ever down, an admin-only Retry button on the warming panel forces one bounded, truthful inline refresh.
- **The Bazarr tab (`/bazarr`) is one Turbo-Frame shell instead of three separate full-page loads.** The header and pill navigation paint instantly; switching between Wanted, Movies and Series swaps only the inner view against the same cache the subtitle badges read, instead of re-fetching Bazarr's full wanted lists (6.2k movies + 13.8k episodes) on every click. URLs, back/forward and pasting a view's own URL into a fresh tab all keep working exactly as before.
- **Ctrl+K search no longer re-transliterates every title on every keystroke.** Titles are normalized once when the 60 s search index is built instead of on every request (and again inside the sort comparator), and the index is sourced from the same library cache the Films page uses instead of fetching Radarr a second time. In an isolated benchmark on a 3.4k-title library the per-request normalization work dropped from 0.8–3.3 s to about 3 ms; warm end-to-end search is expected in the tens of milliseconds (live figures land in `docs/perf/2026-09-01-dev-benchmarks.md` once `:beta` is measured).
- **The dashboard shares the Films/Series library cache instead of keeping its own.** A change made on the Films page — or any library mutation — now shows up on the dashboard immediately instead of waiting up to 45 s for a separate, uncoordinated cache to expire.

### Internal
- **PHPStan level 7 gates `make check` — and therefore CI.** The project had no static-analysis gate, so a whole class of type bug could only be caught by a test that happened to exercise the line. Wiring the analyser into `make check` (which is what CI runs) needed no workflow change. Level 6 was skipped deliberately: over level 5 it adds 812 missing-iterable-annotation errors and *zero* correctness findings, so it is strictly dominated; level 7 adds 68 real type-safety findings on top. The annotation debt is ignored by identifier rather than baselined, so a new file doesn't inherit it, and the remaining baseline (160 entries) is shrink-only. 17 genuine bugs were fixed before the baseline was taken — the headline being that `transliterator_transliterate()` returns `false` on failure, which coerced to `''` and made the library search's `str_contains()` match *every* item, so one bad transliteration silently turned search into "match everything". Also: an undefined-method call in `TmdbController` (the watchlist repository was missing the `@extends ServiceEntityRepository<…>` generic its three siblings declare, so `find()` analysed as bare `object`), five `foreach` loops over `preg_split()`'s `list<string>|false`, a `preg_quote()` with no delimiter argument so it never escaped the `/` of the pattern it feeds, and several `json_encode()` `string|false` returns passed where a string is required. The gate was verified against the bug it exists for: reintroducing the `/search/online` cold-cache pattern is reported as two errors.
- **Resynced with upstream, and adopted its Deluge / Transmission pages.** Upstream took this fork's second contribution wave by rebasing the branches into its `main` and closing the pull requests, then continued on top of them; merging that back down brought in the torrent-page full-reload guard, Classic theme mode, the 4+ instance Radarr/Sonarr sidebar dropdown fix and PHPUnit 14 test hygiene. The Deluge and Transmission **page templates are now upstream's** rather than this fork's: they were the same UI, but upstream's use a properly namespaced `transmission.*` / `deluge.*` translation block where the fork had borrowed 216 `qbittorrent.*` keys, so adopting them removes a permanent ~160-hunk divergence at no visible cost — every string renders identically, verified key-by-key against both catalogues (EN and FR at exact parity, zero missing). The fork's own `TransmissionClient` was kept, because it carries two fixes upstream lacks: the requested `rateDownload`/`rateUpload` fields (without them Transmission returns no speeds and the detail panel reads a flat 0) and PHPStan-clean casts. One hazard worth naming for anyone merging upstream into a hardened fork: upstream still ships `script-src 'unsafe-inline'`, so its templates carry bare `<script>` tags that this fork's enforcing nonce policy refuses to execute — the page renders and every scripted behaviour on it vanishes silently. Re-nonced the adopted templates plus two sidebar pollers that had auto-merged into `base.html.twig`; an inline script with neither `src=` nor a nonce now reports zero repo-wide.
### Added
- **Bazarr integration (fork-only, [#103](https://github.com/ndandan/Prismarr/issues/103)).** A new subtitle-management tab (Wanted / Movies / Series / History) with manual provider search + download and one-click auto-search, plus subtitle-status badges surfaced on the Films/Series grids, the quick-look modal, and global search. Single-instance flat config under `/admin/settings`, admin-only, and fails closed like every other media-client integration.
...
### Changed
- **The Updates settings section tracks this fork instead of upstream's release feed.** `/admin/settings#section-updates` compared the running build against Shoshuo/Prismarr's GitHub releases, whose newest tag predates most of what the fork ships — so it advertised a two-month-old upstream release as an "available upgrade" over a strictly newer build, rendered upstream's release notes as if they described the running software, and printed Docker Hub upgrade instructions for an image the fork doesn't publish. The section is fork-aware end to end now: a truthful **N commits behind fork main** / **Up to date** badge (GitHub's compare API, anchored on the commit SHA the image was stamped with), the fork's own `CHANGELOG.md` rendered as the release notes (Unreleased plus the two most recent released sections), a date-grouped commit feed — one article per day, bucketed *and* labelled in the viewer's timezone rather than UTC, so a commit never lands under a header that contradicts its own timestamp — the `ghcr.io/ndandan/prismarr:latest` pull command plus the Unraid **Force Update** caveat (Apply alone does not re-pull), and upstream demoted to an informational "last release" block carrying no "available" phrasing at all. The About card gains fork source and issues rows above the retained upstream rows, which are now explicitly labelled as upstream. A build with no stamped SHA — a local `make dev` — hides the badge rather than guessing. All three GitHub reads are cached for an hour and a failed read writes a 120 s failure marker, so an outage costs one slow render per two-minute window instead of an 8 s timeout on every single request, and each block degrades to an honest "unavailable" state rather than a wrong number. Full EN + FR strings, pinned by `UpdatesSectionGuardTest`; live-verified on `:beta`.
...
### Security
- **`script-src` drops `'unsafe-inline'` and `data:` for a strict nonce policy, and it is now enforcing.** An injected string can no longer execute, which is the durable fix for the whole XSS class rather than one sink at a time. Every inline `<script>` the app emits — 118 of them — carries a nonce, and all 18 inline `on*=` attribute handlers were converted to `addEventListener` registrations, because a nonce cannot rescue an attribute handler: CSP blocks those unconditionally. The Report-Only stage the policy shipped behind has been retired. `style-src` is deliberately unchanged — inline `style=` attributes are pervasive here and style injection is the strictly lower-severity class, so removing it is separate work with its own risk profile.
- **The nonce is session-stable, not per-request, because per-request rotation breaks Turbo two ways.** One random value per session (under `_csp_nonce`), rotated at the login boundary by a `LoginSuccessEvent` subscriber so the value doesn't outlive the session it was minted for, with a per-request value when there is no session at all. Rotating it per request fails twice over: the importmap polyfill loader is an inline script inside a `data-turbo-track="reload"` head element, so a changing nonce changes that element's *body text* and Turbo forces a full reload on every navigation; and Turbo re-stamps the scripts it re-injects with the newest `<meta name="csp-nonce">` value, which the original document's policy — the one the browser is still enforcing — has never heard of, so those scripts are blocked. Both failures are invisible on a hard refresh, so the `:beta` sweeps navigated in-app; `CspNonceGuardTest`, `PageLifecycleGuardTest` and `CspNonceRenderTest` fail the build if an inline script ever ships without a nonce. Live-verified on `:beta` through a Report-Only round and then an enforcing round.
- **CSP headers are gated to HTML responses.** `CspHeaderSubscriber::governsADocument()` means JSON and API responses no longer carry a policy they cannot use — and, as a side effect, no longer mint an orphan session per cookieless poll. The 30-second Docker healthcheck alone was creating roughly 2,880 session files a day.
- **Three template escaping sweeps closed the XSS sinks that reached the DOM unescaped.** 14 in the Discover detail modal (TMDb-supplied strings), 28 across the three Jellyseerr templates (requester-controlled names), and 17 across the calendar's four render paths (Radarr/Sonarr titles), plus the Explorer's `escHtml` hardened to match its Discover sibling. A 58-row `TemplateEscapingGuardTest` pins each site so a future edit can't quietly drop the escaping again.
- **SABnzbd actions fail closed when the response carries no status.** `SabnzbdClient::action()` defaulted an absent `status` key to `true`, so any unrecognised `200` — a reverse proxy's own page, a different SABnzbd version — was reported to the user as a successful pause / resume / delete / speed-limit / add. Pure actions always carry `status`; its absence is now a failure and is logged, matching the `=== true` strictness `NzbgetClient` already had.
